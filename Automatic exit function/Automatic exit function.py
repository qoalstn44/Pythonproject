...
while True:
    ...
    if timeElapsed > 1:
        ...
        pyautogui.typewrite('chrome')# interval는 사람이 타자를 치는것처럼 시간간격을 두는 함수 0.1은 100ms 으로 한글자를 적을때마다 100ms으로 타핑을 치게된다.
        ...
        for i, name in  enumerate(filename):#enumerate 은 filename안에 있는 리스트들을 index형태로 만들어준다. [ ex) test1.png를 0을로 표시하고 test2.png를 1로 표시한다. ]
            poi = pyautogui.locateCenterOnScreen(name, confidence=0.6) #pyautogui.locateCenterOnScreen는 해당 웹페이지에 좌표를 가져오는 변수이다. confidence는 해당 웹페이지와 png파일의 이미지가 비슷하면 그 좌표를알려준다 0.5니 50%라고 한다.
            print(name, poi)
            if poi != None:
